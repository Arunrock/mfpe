...
public class DrugController
{
    ...
    public DrugDetails getDispatchableDrugStock(@RequestHeader("Authorization") final String token, @PathVariable("id") final String id, @PathVariable("location") final String location) throws InvalidTokenException, StockNotFoundException, DrugNotFoundException {
    ...
    @CrossOrigin
    @ApiOperation(value = "Update quantity by stock", response = ResponseEntity.class)
    @GetMapping({ "/updateDispatchableDrugStock/{id}/{location}/{quantity}" })
    public ResponseEntity<SuccessResponse> AddupdateQuantity(@RequestHeader("Authorization") final String token, @PathVariable("id") final String id, @PathVariable("location") final String location, @PathVariable("quantity") final int quantity) throws InvalidTokenException, DrugNotFoundException, StockNotFoundException {
        return (ResponseEntity<SuccessResponse>)this.drugDetailsService.AddupdateQuantity(id, location, quantity, token);
    }
}