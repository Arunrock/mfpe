...
@FeignClient(url = "http://3.111.214.3:8454/refillappdb", name = "refillapp")
public interface RefillClient {
	...
	@PostMapping("/requestRefillSubscription/{sub_id}/{memberId}/{quantity}/{time}")
	public ResponseEntity<Object> requestRefillSubscription(@RequestHeader("Authorization") final String token,
			...
			@PathVariable("quantity") int quantity, @PathVariable("time") int time);
	...
	@GetMapping("/getBySubscriptionId/{subscriptionId}")
	public boolean getById(@RequestHeader("Authorization") final String token,
			@PathVariable("subscriptionId") Long subscriptionId);
