...
public class RefillOrderServiceImpl implements IRefillOrder {
	...
	public List<RefillOrderLine> getRefillDuesAsOfDate(String memberId, int date,String token) throws InvalidTokenException {
		...
		if (authFeign.getValidity(token).isValid()) {
			List<Long> a = orepo.findByMemberId(memberId);
			...
			System.out.println(memberList.stream().filter(x -> date % x.getRefillTime() != 0&&a.contains(x.getSubID())).collect(Collectors.toList()));
			return memberList.stream().filter(x -> a.contains(x.getSubID())).collect(Collectors.toList());
		}
		else
			throw new InvalidTokenException(msg);
		}
	...
	public RefillOrder requestAdhocRefill(long policyId, long subID, String memberID, String location,int quantity,String token)
		...
		if (authFeign.getValidity(token).isValid()) {
			...
			String[] splited = drugrName.split(" ");
			String name = splited[0];
			log.info(name);
			int refillcycle=Integer.parseInt(splited[1]);
			String subsdate = splited[2];
			...
			if (responseValue == 200) {
				...
				LocalDate date=LocalDate.parse(subsdate).plusDays(refillcycle);
				
				//String str = format.format(date);
				Date date1 = Date.from(date.atStartOfDay(ZoneId.systemDefault()).toInstant());
				order.setRefillDate(date1);
	...
	public boolean getById(long subID,String token) throws InvalidTokenException {
		...
		if(authFeign.getValidity(token).isValid())
		{
			if(orepo.findById(subID).isPresent())
			return true;
			else 
				return false;
		}
